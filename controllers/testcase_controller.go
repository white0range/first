...
func AddTestCase(c *gin.Context) {
	...
	// 2. 呼叫 Service 大脑
	testCase, err := services.AddTestCase(c.Request.Context(), problemIDStr, req)
	if err != nil {
		// 精准拦截不同类型的错误
		switch err.Error() {
		case "invalid_id":
...
func DeleteTestCase(c *gin.Context) {
	...
	// 2. 呼叫 Service 执行抹杀命令
	if err := services.DeleteTestCase(c.Request.Context(), caseID); err != nil {
...
func GetTestCases(c *gin.Context) {
	...
	// 3. 呼叫 Service 大脑
	res, err := services.GetTestCases(c.Request.Context(), problemIDStr, page, limit)
	if err != nil {
