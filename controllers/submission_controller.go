...
func SubmitCode(c *gin.Context) {
	...
	// 呼叫 Service 干活！
	submission, err := services.SubmitCode(c.Request.Context(), userID, req)
	if err != nil {
...
func GetSubmissionResult(c *gin.Context) {
	...
	// 3. 呼叫 Service 大脑查快递！
	submission, err := services.GetSubmissionResult(c.Request.Context(), id, currentUserID)
	if err != nil {
		// 判断是查不到，还是没权限查
...
func GetMySubmissions(c *gin.Context) {
	...
	// 3. 呼叫 Service 大脑
	res, err := services.GetMySubmissions(c.Request.Context(), userID, page, limit)
	if err != nil {
...
func GetAIAssistance(c *gin.Context) {
	...
	stream, err := services.GetAIAssistanceStream(c.Request.Context(), submissionID, userID)
	if err != nil {
